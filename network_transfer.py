"""Authenticated file transfers between TransferDesk instances on a local network.
...
DISCOVERY_MAGIC = "TRANSFERDESK_DISCOVERY_V1"
INTERNET_INVITATION_PREFIX = "TRANSFERDESK1-"
NETWORK_CHUNK_SIZE = 1024 * 1024
MAX_CONTROL_FRAME = 1024 * 1024
MAX_DATAGRAM_SIZE = 64 * 1024
PBKDF2_ROUNDS = 120_000
RESUME_PART_SUFFIX = ".transferdesk-part"
...
class InternetInvitation:
    ...
    tls_fingerprint: str
    expires_at: int
    device_name: str = "TransferDesk PC"
    ...
    def decode(cls, raw_value: str, now: int | None = None) -> InternetInvitation:
        ...
        if not compact.startswith(INTERNET_INVITATION_PREFIX):
            raise NetworkTransferError("Invalid TransferDesk Internet invitation.")
        ...
        try:
            ...
            device_name = str(data.get("n", "TransferDesk PC"))
        except (
            KeyError, ValueError, TypeError, UnicodeError,
            binascii.Error, zlib.error, json.JSONDecodeError,
        ) as exc:
            raise NetworkTransferError("Invalid TransferDesk Internet invitation.") from exc
        ...
        if expires_at <= current_time:
            raise NetworkTransferError("This TransferDesk Internet invitation has expired.")
...
def create_tls_identity(common_name: str = "TransferDesk Internet Transfer") -> TLSIdentity:
    ...
    temporary_directory = tempfile.TemporaryDirectory(prefix="transferdesk-tls-")
...
def _derive_key(code: str, client_nonce: bytes, server_nonce: bytes) -> bytes:
    salt = b"TransferDesk-LAN-v1" + client_nonce + server_nonce
...
class LocalTransferClient:
    """Send a batch of files to one listening TransferDesk instance."""
...
class DiscoveryService:
    """Advertise one running TransferDesk instance and receive transfer requests."""
    ...
    def __init__(
        ...
        self.callback = callback or (lambda _event, _data: None)
        self.port = int(port)
        self.device_name = (device_name or socket.gethostname()).strip() or "TransferDesk PC"
    ...
    def start(self) -> None:
        ...
        self._thread = threading.Thread(target=self._run, daemon=True, name="TransferDeskDiscovery")
        self._thread.start()
...
def discover_devices(
    ...
    """Broadcast a probe and collect running TransferDesk instances."""
...
def create_upnp_mapping(
    ...
    _upnp_action(
        ...
        "AddPortMapping",
        {
            ...
            "NewEnabled": 1,
            "NewPortMappingDescription": "TransferDesk Internet Transfer",
...
NETWORK_TEXT = {
    "fr": {
        ...
        "nearby_pcs": "PC à proximité avec TransferDesk ouvert",
        "refresh": "Rechercher",
        "scanning": "Recherche des PC TransferDesk sur le réseau…",
        "no_devices": "Aucun autre PC TransferDesk détecté.",
        ...
        "incoming_title": "Demande de transfert TransferDesk",
        ...
        "internet_intro": "Entre deux maisons : TransferDesk cherche une connexion directe, puis utilise un relais sécurisé seulement si le réseau l’impose.",
        ...
        "missing_rendezvous": "Configure d’abord l’adresse du service de connexion TransferDesk.",
        ...
        "private_lan": "Les PC avec TransferDesk ouvert apparaissent automatiquement sur ce réseau local privé.",
    },
    "en": {
        ...
        "nearby_pcs": "Nearby PCs with TransferDesk open", "refresh": "Scan",
        "scanning": "Looking for TransferDesk PCs on the network…",
        "no_devices": "No other TransferDesk PC was found.",
        ...
        "incoming_title": "TransferDesk transfer request",
        ...
        "internet": "Internet P2P",
        "internet_intro": "Between two homes: TransferDesk tries a direct route first, then uses a secure relay only when required by the network.",
        ...
        "missing_rendezvous": "Configure the TransferDesk connection service address first.",
        ...
        "private_lan": "PCs with TransferDesk open appear automatically on this private local network.",
    },
}
