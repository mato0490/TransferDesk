"""Universal encrypted AutoSD transfers using WebRTC ICE/STUN/TURN.
...
MANUAL_SIGNAL_PREFIX = "AUTOSD-MANUAL-1."
...
def default_rendezvous_url() -> str:
    ...
    environment_value = os.environ.get("AUTOSD_RENDEZVOUS_URL", "").strip()
    ...
    for root in roots:
        for filename in ("autosd-network.json",):
...
def _connection_timeout_message(servers: tuple[IceServer, ...]) -> str:
    message = (
        ...
        "la même version d’AutoSD, autorise AutoSD dans le pare-feu Windows et "
...
class RendezvousClient:
    """Small HTTP client for an AutoSD-compatible rendezvous service."""
    ...
    def _request(
        ...
        *,
        ...
        body = None
        headers = {"Accept": "application/json", "User-Agent": "AutoSD/8"}
...
async def _receive_files(
    ...
    if offer.get("version") != WEBRTC_PROTOCOL_VERSION:
        await inbox.send_control("error", message="Version AutoSD incompatible.")
        raise UniversalTransferError("Version AutoSD incompatible.")
    ...
    for index, (name, expected_size) in enumerate(manifest):
        ...
        temporary = destination / f".{final_path.name}.autosd-part-{secrets.token_hex(5)}"
        digest = hashlib.sha256()
        received = 0
        try:
...
class ManualSender:
    ...
    async def _send_files(self, paths: list[Path]) -> UniversalTransferResult:
        ...
        channel = pc.createDataChannel("autosd-files", ordered=True)
